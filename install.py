...
def main() -> None:
    ...
    if not args.skip_precompute:
        ...
        if has:
            ...
            print("  Options:")
            print("    - Download release tarball (~1.4 GB), no `gh` required:")
            print("        curl -L https://github.com/hchen13/poker-gto-engine/releases/download/v0.1/precompute_out_v2.tar.zst \\")
            print("          | zstd -d | tar -x")
            print("    - Or with `gh` CLI:")
            print("        gh release download v0.1 --pattern 'precompute_out_v2.tar.zst'")
            print("        zstd -d precompute_out_v2.tar.zst -c | tar -x")
            print("    - Generate locally (~14 h, requires built binaries):")
